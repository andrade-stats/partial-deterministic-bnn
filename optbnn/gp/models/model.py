...
from .. import mean_functions, parameter
    ...
    def sample_functions(self, X, num_samples):
        ...
        if len(prior_params) == 0:
            ...
            for i in range(self.num_latent):
                ...
                L = torch.linalg.cholesky(var + jitter, upper=False)
            ...
            for sample_idx in range(num_samples):
                for param in prior_params:
                    with torch.no_grad():
                        # print(type(param))
                        # print(type(param.prior.sample()))                        
                        if not (param.shape == param.prior.sample().shape):
                            print(f'param.shape = {param.shape}')
                            print(f'param.prior.sample().shape = {param.prior.sample().shape}')
                            False
                ...
                for i in range(self.num_latent):
                    ...
                    while True:
                        try:
                            ...
                            L = torch.linalg.cholesky(var + multiplier*jitter, upper=False)
                            break
