...
from ..metrics.metrics_tensor import accuracy, nll
...
class ClassificationNet(BayesNet):
    def __init__(self, net, likelihood, prior, ckpt_dir, temperature=1.0,
                 sampling_method="adaptive_sghmc", logger=None, n_gpu=None):
    ...
    def log_predict(self, x_test, all_sampled_weights=True,
        ...
        # Estimate the predictive mean
        # mean_predictions = torch.mean(predictions, axis=0)
        mean_predictions = torch.logsumexp(predictions, dim=0) - torch.log(torch.tensor(predictions.shape[0])) #!
        mean_predictions = mean_predictions.to(self.device)
    ...
    def _print_evaluations(self, x, y, train=True):
        ...
        """
        preds = self.log_predict(x, all_sampled_weights=(not train)) #!
